...
numident_mr <- numident %>% 
  ...
  mutate(total_deaths = sum(n) * 1.05352238) %>% ## estimated from hmd exposure 
...
numident_mortality_validation <- hmd_cohort_1910 %>% 
  ...
  bind_rows(numident_mr_adjusted %>% 
              filter(sex == 1)) %>% 
   bind_rows(numident_mr %>% 
              filter(sex == 1)) %>% 
...
dmf_mort <- dmf %>% 
  ...
  group_by(death_age) %>% 
  summarize(n = sum(weight_conservative)) %>% 
  mutate(total_deaths = sum(n) * 1.02705555) %>% ## estimated from hmd exposure 
  mutate(cum_deaths = cumsum(n)) %>% 
  mutate(survivors = total_deaths - lag(cum_deaths)) %>% 
  mutate(male = n / survivors) %>% 
  dplyr::select(death_age, male) %>% 
